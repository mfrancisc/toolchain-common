...
func TestGetOwnersFailures(t *testing.T) {
	...
	t.Run("api not available", func(t *testing.T) {
		// given
		pod := givenPod.DeepCopy()
		err := controllerruntime.SetControllerReference(vm, pod, scheme.Scheme)
		require.NoError(t, err)
		dynamicClient := fakedynamic.NewSimpleDynamicClient(scheme.Scheme, pod, vm)

		fakeDiscovery := newFakeDiscoveryClient(apiSchemeResourceList(t)...)
		fetcher := NewOwnerFetcher(fakeDiscovery, dynamicClient)
		...
		// then
		require.EqualError(t, err, "no resource found for kind VirtualMachine in kubevirt.io/v1")
